This tutorial gives example input files and instructions for a typical unguided graph-\/driven sampling (G\+DS) calculation run through C\+DE.

In this case, we are going to run a G\+DS simulation for the 4-\/atom formaldehyde system. The aim of these simulations is to stochastically generate a large set of reactions (using the allowed move classes in the {\itshape movefile}). After generation of a set of reactions, further analysis such as N\+EB or Graph\+Gen can be used to study the reaction network.

Ultimately, one might also perform reaction rate calculations and use the rates as input to a microkinetic modelling simulation; connecting C\+DE to a kinetics modelling code is a work in progress (\mbox{\hyperlink{todo}{Things to do}}).

{\bfseries{The actual files to run this example can be found in the $\ast$$\sim$/cde/examples/\+Tutorial\+\_\+4/$\ast$ directory.}}
...
Let\textquotesingle{}s have a look at each input file individually\+:\hypertarget{_tutorial4_autotoc_md32}{}\doxysubsection{input}\label{_tutorial4_autotoc_md32}
...
The input file in this case only contains those options which are relevant to the G\+DS simulation; other input directives have been removed and will be ignored internally in C\+DE.
...
\begin{DoxyItemize}
\item {\bfseries{nimage}}\+: Number of images (in total) in the reaction-\/path which is going to be used to sample reaction-\/space. In this case, we have 8.
...
{\bfseries{I\+M\+P\+O\+R\+T\+A\+NT\+: If the {\itshape pestype} is set to null (as it is in this example), the G\+DS reaction-\/string is not really dynamic anymore, and we instead simply generate reaction paths connecting sampled reactants and products using interpolation (rather than molecular dynamics).}}
...
\item {\bfseries{endfile}}\+: Like the {\itshape startfile}, the {\itshape endfile} is an X\+YZ file which contains the coordinates of the end-\/point of the initial reaction-\/string. This file must contain the same number of atoms, as well as the same atomic label order, as the {\itshape startfile}.
\end{DoxyItemize}

{\bfseries{Note that the {\itshape startfile} and the {\itshape endfile} can be identical; after starting with this \char`\"{}zero-\/string\char`\"{}, the natural evolution of G\+DS will quickly start generating new reaction-\/paths.}}
...
Note that there is some redundancy here, but the ability to define reactivity in different ways allows flexibility in applications.\hypertarget{_tutorial4_autotoc_md33}{}\doxysubsection{start.\+xyz and end.\+xyz}\label{_tutorial4_autotoc_md33}
...
If {\itshape start.\+xyz} and {\itshape end.\+xyz} are different, the code will try to generate an initial reaction-\/string by linear interpolation between the two end-\/points if the variable {\itshape pathinit} is set to {\itshape linear}. (Note that, in the current version of the code, the only option to {\itshape pathinit} is {\itshape linear}).\hypertarget{_tutorial4_autotoc_md34}{}\doxysubsection{moves.\+in}\label{_tutorial4_autotoc_md34}
...
The format of the move-\/file is discussed in \mbox{\hyperlink{moves}{Movefile example}}.\hypertarget{_tutorial4_autotoc_md35}{}\doxysubsection{Running the calculation}\label{_tutorial4_autotoc_md35}
...
In addition to the above ouptut files, the $\ast$.log file produced by all C\+DE calculations will also contain some useful information about the N\+EB run.\hypertarget{_tutorial4_autotoc_md36}{}\doxysubsection{Next steps}\label{_tutorial4_autotoc_md36}
