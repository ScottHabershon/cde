...
! stop
    ...
    Call PrintMechanismPaths(nrxn, cx_start, cx, file_root, maxbarrier, bsum, cx_end)
...
!      enddo
  ...
  !> GraphsToCoords_BD
  !!
  !! Converts a pair of connectivity graphs generated by RunBreakdown to xyz coordinates 
  !! by optimization under the action of the graph-restraining potential.
  !!
  ...
  Subroutine PrintMechanismPaths(nrxn, cx_start, cx, file_root, maxbarrier, bsum, cx_end)
    ...
    integer :: nrxn, na, irxn, j, idum
    type(rxp) :: rp
    type(cxs) :: cx_start, cx(nrxn)
    type(cxs), optional :: cx_end
    character (len=7) :: x1
    character (len=8) :: fmt
    character (len=20) :: file_root
    ...
    outer3: do irxn = 1, nrxn+1
      ...
      ! Optionally, also include the final reaction to the endpoint if requested.
      !
      elseif (irxn == nrxn+1) then
        if (do_final) then
          rp%cx(1) = cx(nrxn)
          do j = 2, nimage-1
            rp%cx(j) = cx(nrxn)
  ...
  subroutine RunNetGrow2()
    ...
    mcloop: do irx = 1, nmcrxn
      ...
      rxloop: do istep = 1, nrxn
        ...
        ! write(logfile, '("* Relaxing geometries of currently generated reaction steps on double-ended GRP ONLY.")')
        ! call GraphsToCoords(cx_start, wcx, istep, .false., '', .false.)
  ...
  !!
  !! Runs automatic growth of a reaction network, based on allowed
  !! graph-moves. Generates nmcrxn random reaction pathways sequentially,
  !! with stepwise optimisation (if enabled).
  !!
  !************************************************************************
